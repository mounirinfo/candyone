  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setCallbacks(dataCoach);

        const resAdmin = await fetch("/api/admin");
        const dataAdmin = await resAdmin.json();
        setAbonnements(dataAdmin);
                  ...
                  <TableBody>
                    {callbacks.map((cb) => (
                      <TableRow key={cb.id} hover>
                        ...
                        <TableCell>
                          <Typography variant="body2">{cb.telephone}</Typography>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <Chip 
                            ...
                            sx={{ 
                              backgroundColor: cb.notes_interne ? Pink + '20' : 'transparent',
                              color: cb.notes_interne ? Pink : 'text.secondary',
                              borderColor: Pink
                            }}
                          />
                        </TableCell>
                    ...
                    {abonnements.map((abo) => (
                      <TableRow key={abo.id} hover>
                        <TableCell>
                          <Typography variant="body2" fontWeight="medium">
                            {abo.contrat?.client
                              ? `${abo.contrat.client.prenom} ${abo.contrat.client.nom}`
                              : "Inconnu"
                            }
                          </Typography>
                        </TableCell>
                        ...
                        <TableCell>
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            <Typography variant="body2">
                              {abo.date_debut} {abo.date_fin && `- ${abo.date_fin}`}
                            </Typography>
                          </Box>
                        </TableCell>
                        <TableCell>
